...
1. characteristics
2. immunophenotype
3. \`developmental stage\` (As there is a space in the column name we need to use backticks around the name (\`\`). Note that backticks are not single quotes (\'\'). The backtick button is usually at the top left corner of a laptop keyboard under the ESC key. Check what happens if you don't use backticks.)
...
ggplot(data = allinfo, mapping = aes(x = Sample, y = log2(Count + 1), fill = characteristics)) + 
  geom_boxplot() 

# Plot immunophenotype
ggplot(data = allinfo, mapping = aes(x = Sample, y = log2(Count + 1), fill = immunophenotype)) + 
  geom_boxplot()

# Plot developmental stage
ggplot(data = allinfo, mapping = aes(x = Sample, y = log2(Count + 1), fill = `developmental stage`)) + 
  geom_boxplot() 
```
