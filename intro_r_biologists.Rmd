...
To install the tidyverse package you type `install.packages('tidyverse')`.  
...
We will use the counts file called `GSE60450_GeneLevel_Raw_data.csv` that's in a folder called `data` i.e. the path to the file should be `data/GSE60450_GeneLevel_Raw_data.csv`.
...
The value of `counts` is the contents of the counts file. There is some information output by `read_tsv` on column specifications, this is the data type that `read_tsv` has guessed is contained in each column, we will discuss this more later. When assigning a value to an object, R does not print the value. For example, here we don't see what's in the counts file. But there are ways we can look at the data. 

In the Environment tab we can see how many rows and columns the object contains and we can click on the icon to view the file. This runs the command View() for us.

# Formatting the data

```{r}
# format - convert to long/tidy format
