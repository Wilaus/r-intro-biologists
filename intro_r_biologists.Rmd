...
Alternatively, we could specify the columns we *don't* want to reformat and gather will reformat all the other columns. To do that we put a hyphen "-" in front of the column names that we don't want to reformat. This is a common way to use `gather()` as sometimes it is easier to do that, for example, if the sample ids didn't all start with the same prefix "GSM". The command below would give us the same result as the previous command.
...
Now that we've got just one column containing sample ids in both our counts file and our metadata files we can join them together using the sample ids.
...
1. characteristics
2. immunophenotype
3. \`developmental stage\` (As there is a space in the column name we need to use backticks around the name (\`). Note that backticks are not single quotes (\'). hTe backtick button is usually at the top left of a laptop keyboard under the ESC button. Check what happens if you don't use backticks.)
...
The boxplots don't look good as we only have two values per group. We could just plot the individual points instead. We could use 
...
The points are overlapping so we will use `geom_jitter()` which adds a small amount of random variation to the points.
